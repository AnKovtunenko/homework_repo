    ...
    if (CART.length) {
        CART.toSorted((a, b) => a.isBought - b.isBought).forEach((item, index) => {
            ...
            html += `<tr data-title="${item.title}" >
                ...
                <td>${total}</td>
                <td> 
                ${!item.isBought ? '<button class="button is-small is-success" onclick="buyProduct(\'' + item.title + '\')">Buy</button>' : ''}
            </tr > `
...
function buyProduct(title) {
    const foundIndex = CART.findIndex((element) => element.title.toLowerCase() === title.toLowerCase());
    if (foundIndex > -1) {
        CART[foundIndex].isBought = true;
    }
    showProductsList()
}
